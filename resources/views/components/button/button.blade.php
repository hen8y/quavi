...
<button 
    ...
    {!! $attributes->merge(['class' => "
        {$themeClasses['button']}
        shadow-lg mt-2 block cursor-pointer
        {$sizeClasses['width']} 
        rounded-full p-0.5
        {$class}
    "]) !!}
    ...
    <div 
        ...
        wire:loading.flex 
        wire:target="{{ $target }}"
        class="{{ $themeClasses['loading'] }} rounded-full size-full gap-2 {{ $sizeClasses['padding'] }} cursor-progress items-center justify-center {{ $loadingStateClass }}">
    ...
    <div 
        wire:loading.remove 
        wire:target="{{ $target }}"
        class="text-center {{ !$disabled ?: 'opacity-60 cursor-not-allowed' }} {{ $themeClasses['default'] }} rounded-full size-full gap-2 {{ $sizeClasses['padding'] }}">
